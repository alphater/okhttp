 ...
 * <p>
 * Note that this class can be used in two roles: first, as a static utility class for loading purposes, and second, as a singleton loader of
 * per-module version information.
 * <p>
 * Note that method for accessing version information changed between versions 2.1 and 2.2; earlier code used file named "VERSION.txt"; but
 * this has serious performance issues on some platforms (Android), so a replacement system was implemented to use class generation and
...
public class VersionUtil {
	...
	protected VersionUtil() {
		...
		try {
			/*
			 * Class we pass only matters for resource-loading: can't use this Class (as it's just being loaded at this point), nor anything
			 * that depends on it.
			 */
	...
	/**
	 * Will attempt to load the maven version for the given groupId and artifactId. Maven puts a pom.properties file in
	 * META-INF/maven/groupId/artifactId, containing the groupId, artifactId and version of the library.
	 * 
	 * @param cl
	...
	public static Version mavenVersionFor(ClassLoader cl, String groupId, String artifactId) {
		InputStream pomProperties = cl.getResourceAsStream("META-INF/maven/" + groupId.replaceAll("\\.", "/") + "/" + artifactId
				+ "/pom.properties");
	...
	public static Version parseVersion(String s, String groupId, String artifactId) {
		if (s != null && (s = s.trim()).length() > 0) {
			...
			return new Version(parseVersionPart(parts[0]), (parts.length > 1) ? parseVersionPart(parts[1]) : 0,
					(parts.length > 2) ? parseVersionPart(parts[2]) : 0, (parts.length > 3) ? parts[3] : null, groupId, artifactId);
		}
		return null;
	}
