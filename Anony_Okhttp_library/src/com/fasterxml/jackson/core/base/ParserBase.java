...
/**
 * Intermediate base class used by all Jackson {@link JsonParser} implementations. Contains most common things that are independent of actual
...
public abstract class ParserBase extends ParserMinimalBase {
	...
	/**
	 * Flag that indicates whether parser is closed or not. Gets set when parser is either closed by explicit call ({@link #close}) or when
	...
	/**
	 * Current index of the first character of the current row in input buffer. Needed to calculate column position, if necessary; benefit of
	 * not having column itself is that this only has to be updated once per line.
	 */
	...
	/**
	 * Total number of bytes/characters read before start of current token. For big (gigabyte-sized) sizes are possible, needs to be long,
	...
	/**
	 * Secondary token related to the next token after current one; used if its type is known. This may be value token that follows
	...
	/**
	 * Buffer that contains contents of String values, including field names if necessary (name split across boundary, contains escape
	 * sequence, or access needed to char array)
	 */
	protected final TextBuffer _textBuffer;

	/**
	 * Temporary buffer that is needed if field name is accessed using {@link #getTextCharacters} method (instead of String returning
	 * alternatives)
	 */
	protected char[] _nameCopyBuffer = null;

	/**
	 * Flag set to indicate whether the field name is available from the name copy buffer or not (in addition to its String representation
	 * being available via read context)
	 */
	...
	/**
	 * We will hold on to decoded binary data, for duration of current event, so that multiple calls to {@link #getBinaryValue} will not need
	 * to decode data more than once.
	 */
	...
	/**
	 * Flag that indicates whether numeric value has a negative value. That is, whether its textual representation starts with minus
	 * character.
	 */
	 ...
	 *        Bitflag of standard features after they were changed
	 * @param changedFeatures
	 *        Bitflag of standard features for which setting did change
	 * 
	 * @since 2.7
	 */
	...
	/**
	 * Method that return the <b>starting</b> location of the current token; that is, position of the first character from input that starts
	 * the current token.
	 */
	@Override
	...
	/**
	 * Method called when an EOF is encountered between tokens. If so, it may be a legitimate EOF, but only iff there is no open non-root
	 * context.
	 */
	@Override
	...
	protected void _reportMismatchedEndMarker(int actCh, char expCh) throws JsonParseException {
		...
		_reportError("Unexpected close marker '" + ((char) actCh) + "': expected '" + expCh + "' (for " + _parsingContext.getTypeDesc()
				+ " starting at " + startDesc + ")");
	}
	...
	public NumberType getNumberType() throws IOException {
		...
		/*
		 * And then floating point types. Here optimal type needs to be big decimal, to avoid losing any data? However... using BD is slow, so
		 * let's allow returning double as type if no explicit call has been made to access data as BD?
		 */
		if ((_numTypesValid & NR_BIGDECIMAL) != 0) {
	...
	/**
	 * Method that will parse actual numeric value out of a syntactically valid number value. Type it will parse into depends on whether it is
	 * a floating point number, as well as its magnitude: smallest legal type (of ones available) is used for efficiency.
	 * 
	 * @param expType
	...
	private void _parseSlowFloat(int expType) throws IOException {
		/*
		 * Nope: floating point. Here we need to be careful to get optimal parsing strategy: choice is between accurate but slow (BigDecimal)
		 * and lossy but fast (Double). For now let's only use BD when explicitly requested -- it can still be constructed correctly at any
	...
	protected void convertNumberToDouble() throws IOException {
		/*
		 * 05-Aug-2008, tatus: Important note: this MUST start with more accurate representations, since we don't know which value is the
	...
	/**
	 * Method that sub-classes must implement to support escaped sequences in base64-encoded sections. Sub-classes that do not need base64
	 * support can leave this as is
	 */
