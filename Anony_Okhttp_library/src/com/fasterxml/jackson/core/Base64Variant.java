...
/**
 * Abstract base class used to define specific details of which variant of Base64 encoding/decoding is to be used. Although there is somewhat
 * standard basic version (so-called "MIME Base64"), other variants exists, see <a href="http://en.wikipedia.org/wiki/Base64">Base64 Wikipedia
 * entry</a> for details.
 * 
...
public final class Base64Variant implements java.io.Serializable {
	...
	/**
	 * Marker used to denote ascii characters that do not correspond to a 6-bit value (in this variant), and is not used as a padding
	 * character.
	 */
	...
	/**
	 * Maximum number of encoded base64 characters to output during encoding before adding a linefeed, if line length is to be limited (
	...
	/**
	 * "Copy constructor" that can be used when the base alphabet is identical to one used by another variant except for the maximum line
	 * length (and obviously, name).
	 */
	...
	/**
	 * "Copy constructor" that can be used when the base alphabet is identical to one used by another variant, but other details (padding,
	 * maximum line length) differ
	 */
	...
	/**
	 * Method that outputs partial chunk (which only encodes one or two bytes of data). Data given is still aligned same as if it as full
	 * data; that is, missing data is at the "right end" (LSB) of int.
	 * 
	 * @param outputBytes
	...
	/**
	 * Convenience method for converting given byte array as base64 encoded String using this variant's settings. Resulting value is "raw",
	...
	/**
	 * Convenience method for converting given byte array as base64 encoded String using this variant's settings, optionally enclosed in
	 * double-quotes.
	 * 
	 * @param input
	 *        Byte array to encode
	...
	/**
	 * Convenience method for decoding contents of a Base64-encoded String, using this variant's settings and appending decoded binary data
	 * using provided {@link ByteArrayBuilder}.
	 * <p>
	 * NOTE: builder will NOT be reset before decoding (nor cleared afterwards); assumption is that caller will ensure it is given in proper
	 * state, and used as appropriate afterwards.
	 * 
	 * @since 2.2.3
	 * 
	 * @throws IllegalArgumentException
