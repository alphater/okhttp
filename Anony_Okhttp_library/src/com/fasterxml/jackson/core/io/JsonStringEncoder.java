 ...
 * <p>
 * Note that methods in here are somewhat optimized, but not ridiculously so. Reason is that conversion method results are expected to be
 * cached so that these methods will not be hot spots during normal operation.
 */
public final class JsonStringEncoder {
	...
	private static int _convert(int p1, int p2) {
		...
		if (p2 < SURR2_FIRST || p2 > SURR2_LAST) {
			throw new IllegalArgumentException("Broken surrogate pair: first char 0x" + Integer.toHexString(p1) + ", second 0x"
